...
import { Renderer } from '../rendering/content.renderer';
...
export class TaskHydrationService implements ITaskHydrationUseCase {
  ...
  hydrateTask(task: ITask, dddKitPath: string, targetPath: string, pin?: string): Promise<void> {
    const log = getLogger();
    // @ts-expect-error - Parameters are used in legacy wrapper function
    const _dddKitPath = dddKitPath;
    // @ts-expect-error - Parameters are used in legacy wrapper function
    const _targetPath = targetPath;
